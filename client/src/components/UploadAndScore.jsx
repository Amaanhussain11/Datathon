      ...
      <div className="row">
        <div className="col">
          <div className="btn-group">
            <button onClick={() => setJsonText(JSON.stringify(DEMOS.salaried, null, 2))}>Load Salaried</button>
            <button onClick={() => setJsonText(JSON.stringify(DEMOS.gig, null, 2))}>Load Gig</button>
            <button onClick={() => setJsonText(JSON.stringify(DEMOS.risky, null, 2))}>Load Risky</button>
          </div>
          <div className="btn-group" style={{ alignItems: 'center' }}>
            <input type="file" accept="image/*" onChange={e => setImageFile(e.target.files?.[0] || null)} />
            <button disabled={loading || !imageFile} onClick={extractFromImage}>Extract from Image</button>
          </div>
          <textarea value={jsonText} onChange={e => setJsonText(e.target.value)} rows={18} spellCheck={false} />
          <div className="btn-group">
            <button disabled={loading} onClick={() => callServer('ml')}>Calculate Score (ML)</button>
            <button disabled={loading} onClick={() => callServer('fallback')}>Calculate Score (Fallback)</button>
          </div>
          {loading && <p>Calculating...</p>}
          {error && <p className="error">{error}</p>}
          {ocrInfo && <p style={{ color:'#9db3e6' }}>{ocrInfo}</p>}
        </div>
        <div className="col">
